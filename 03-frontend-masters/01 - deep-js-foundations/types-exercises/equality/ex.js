// TODO: write `findAll(..)`
// subset of coercive matching - eliminating corner cases - can use abstract equality sign (==)

const findAll = (match, arr) => {
  for (val of arr) {
    if (Object.is(match, val)) {
      result.push(val);
    } else if (val == null && match == null) {
      // null and undefined case
      result.push(val);
    } else if (typeof match == "boolean" && typeof val === "boolean") {
      // strict match of booleans
      if (match == val) {
        result.push(val);
      }
    } else if (
